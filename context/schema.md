...
- **Bundled CSV:** header with `Timestamp` (unix **seconds**, float ok) + `Open`,`High`,`Low`,`Close`,`Volume` — see [`src/historical_data/mod.rs`](../src/historical_data/mod.rs). Optional: Kaggle / [kagglehub](https://github.com/Kaggle/kagglehub) + unzip.
- **Binance REST JSON:** **`fetch_max_btcusdt_1m`** → root **`candles`** + **`bar_interval`**; **`binance-fetch`** may still emit **`candles_15m`** (legacy JSON key, not TF) — serde alias on **`MachineRequest`** in [`src/machine.rs`](../src/machine.rs).
